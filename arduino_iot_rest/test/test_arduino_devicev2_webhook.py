# coding: utf-8

"""
    Iot API

    Collection of all public API endpoints.  # noqa: E501

    The version of the OpenAPI document: 2.0
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest

import arduino_iot_rest
from arduino_iot_rest.models.arduino_devicev2_webhook import ArduinoDevicev2Webhook  # noqa: E501
from arduino_iot_rest.rest import ApiException


class TestArduinoDevicev2Webhook(unittest.TestCase):
    """ArduinoDevicev2Webhook unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testArduinoDevicev2Webhook(self):
        """Test ArduinoDevicev2Webhook"""
        # FIXME: construct object with mandatory attributes with example values
        # model = arduino_iot_rest.models.arduino_devicev2_webhook.ArduinoDevicev2Webhook()  # noqa: E501
        pass


if __name__ == '__main__':
    unittest.main()
